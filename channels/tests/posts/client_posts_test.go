...
func TestClient_ChannelPosts(t *testing.T) {
	...
	t.Run("Test channel request validation", func(t *testing.T) {
		...
		testServer.Mux.HandleFunc(endpoints.ChannelsPosts, func(w http.ResponseWriter, r *http.Request) {
			...
			json.NewEncoder(w).Encode(tgstat.ChannelPostsResult{
				Status: "ok",
...
func TestClient_ChannelPostsExtended(t *testing.T) {
	RegisterTestingT(t)
	t.Run("Test channel request validation", func(t *testing.T) {
		testServer := server.NewServer()
		defer testServer.Teardown()
		...
		_, _, err := channels.PostsExtended(context.Background(), request)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("ChannelId: cannot be blank"))
	})
	...
	t.Run("Test channel posts response Mapping", func(t *testing.T) {
		...
		testServer.Mux.HandleFunc(endpoints.ChannelsPosts, func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			items := make([]tgstat.ChannelPostsWithChannelResponseItem, 0)
			items = append(items, tgstat.ChannelPostsWithChannelResponseItem{
				ID:            7377,
				Date:          1540123429,
				Views:         148382,
				Link:          "t.me/breakingmash",
			...
			json.NewEncoder(w).Encode(tgstat.ChannelPostsWithChannelResult{
				Status: "ok",
				Response: tgstat.ChannelPostsWithChannelResponse{
					Count:      0,
					TotalCount: 0,
					Channel:    channel,
					Items:      items,
		...
		response, _, err := channels.PostsExtended(context.Background(), request)
